class Solution {
	...
	public void constructTrie(int num, Node current) {
		for(int i=30; i>=0; i--) {
			int value =(num>>i)&1;          //Extract the current bit
			if(value==0){               //If it is zero then push it towards left
				if(current.left==null)
					current.left=new Node(0);
				current=current.left;
			} else{
				if(current.right==null)         //If it is one then push it towards right 
