class Solution {
    ...
    void findAllSubstrings(String s, ArrayList<String> temp){
        int n = s.length();
        for(int k=1; k<n; k++){
            String left = s.substring(0, k);    //To optimise left recurive call further than normal dp 
            if( isPalindromeAlready(left) ){
                temp.add(left);

                String right = s.substring(k, n);  //To optimise right recurive call further than normal dp 
